...
import { FiUpload, FiFile, FiImage, FiMusic, FiVideo, FiX, FiCheck } from 'react-icons/fi';
  ...
  const uploadFiles = async () => {
    ...
    for (const fileItem of files) {
      try {
        ...
        toast.error(`Yuklab bo'lmadi: ${fileItem.file.name}`);
      }
    }
    setUploading(false);
    setTimeout(() => {
    ...
    <div className="w-full">
      ...
      >
        ...
        {isDragActive ? (
          ...
          <div>
            <p className="text-gray-600">Drag & drop yoki tanlash uchun bosing</p>
            <p className="text-sm text-gray-400 mt-1">Rasm, audio, video, PDF (max 50MB)</p>
          </div>
        )}
      </div>
        ...
        {files.length > 0 && (
          ...
          >
            ...
            <button
              onClick={uploadFiles}
              disabled={uploading}
              className="w-full py-2 font-semibold text-white transition rounded-lg bg-primary hover:bg-primary/90 disabled:opacity-50"
            >
