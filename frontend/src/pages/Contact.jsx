...
export default function Contact() {
        ...
        <div className="grid gap-10 lg:grid-cols-2">
          ...
          <div className="space-y-6 perspective-1000">
              ...
              >
                <div className="flex items-center gap-5 p-5 transition-all duration-500 border shadow-xl bg-white/80 backdrop-blur-sm rounded-2xl hover:shadow-2xl hover:-translate-y-2 group-hover:bg-white border-white/20">
                  <div className="relative flex items-center justify-center overflow-hidden transition-all duration-500 w-14 h-14 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-xl group-hover:scale-110 group-hover:rotate-3">
                    <i className={`fas ${info.icon} text-2xl text-[#003580] transition-all duration-500 group-hover:scale-110`}></i>
            ...
            <div className="relative p-6 overflow-hidden transition-all duration-500 bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 rounded-2xl group hover:scale-[1.02] animate-float">
              ...
              <h3 className="relative mb-3 text-lg font-bold text-gray-800">{t('Ijtimoiy tarmoqlar', 'Социальные сети')}</h3>
              <div className="relative flex gap-4">
                {[
                  { icon: 'fab fa-telegram', color: 'bg-[#0088cc]', link: 'https://t.me/Asbek_a1' },
                  { icon: 'fab fa-facebook-f', color: 'bg-[#1877f2]', link: 'https://facebook.com' },
                  { icon: 'fab fa-instagram', color: 'bg-[#e4405f]', link: 'https://instagram.com/_asil_bek_07' },
                  { icon: 'fab fa-youtube', color: 'bg-[#ff0000]', link: 'https://www.youtube.com/@jondortumanhokimligi6932' }
                ].map((social, i) => (
                  <a 
                    key={i}
                    href={social.link} 
            ...
            <div className="p-6 border shadow-2xl bg-white/90 backdrop-blur-sm rounded-2xl md:p-8 border-white/20">
              {submitted ? (
                <div className="py-12 text-center animate-bounce-in">
                  ...
                  <h3 className="mb-2 text-2xl font-bold text-gray-800">{t('Murojaatingiz qabul qilindi!', 'Ваше обращение принято!')}</h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  {['name', 'email', 'phone', 'subject'].map((field, idx) => (
                    <div key={field} className="group animate-fade-in-up" style={{ animationDelay: `${idx * 100}ms` }}>
                      <label className="block mb-1 text-sm font-medium text-gray-700">
                        {field === 'name' ? t('Ism familiya *', 'Имя фамилия *') : 
                         field === 'email' ? 'Email *' :
                         field === 'phone' ? t('Telefon', 'Телефон') : t('Mavzu', 'Тема')}
                      </label>
                      <input 
                  ...
                  <div className="group animate-fade-in-up" style={{ animationDelay: '400ms' }}>
                    <label className="block mb-1 text-sm font-medium text-gray-700">{t('Xabar *', 'Сообщение *')}</label>
                    <textarea 
                      rows="5" 
                      required 
                  ...
                  >
                    ...
                    <div className="relative flex items-center justify-center gap-2">
                      {loading ? <i className="fas fa-spinner fa-spin"></i> : <i className="transition-transform fas fa-paper-plane group-hover/btn:translate-x-1 group-hover/btn:-translate-y-1"></i>}
        ...
        <div className="relative overflow-hidden transition-all duration-500 border shadow-lg bg-white/90 backdrop-blur-sm rounded-2xl hover:shadow-2xl hover:-translate-y-3 border-blue-100/50">
          ...
          <div className="relative z-10 p-6">
            ...
            <div className="flex items-start justify-between mb-4">
              <div className={`relative w-16 h-16 bg-gradient-to-br from-[#003580] to-[#0066cc] rounded-2xl flex items-center justify-center shadow-lg transition-all duration-500 group-hover:scale-110 group-hover:rotate-6 overflow-hidden`}>
                <i className={`fas ${org.icon} text-white text-2xl relative z-10`}></i>
            ...
            <div className="mb-4">
              ...
              <p className="mt-1 text-xs text-gray-400">{org.fullName}</p>
              <p className="mt-3 text-sm leading-relaxed text-gray-600">{org.description}</p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 py-3 mb-4 border-t border-b border-blue-100">
              <div className="text-center">
                <div className="text-lg font-bold text-[#003580]">10k+</div>
                <div className="text-xs text-gray-400">{t('Foydalanuvchi', 'Пользователей')}</div>
              </div>
              <div className="text-center">
                <div className="text-lg font-bold text-[#003580]">99.9%</div>
                <div className="text-xs text-gray-400">{t('Ishonchlilik', 'Надежность')}</div>
              </div>
              <div className="text-center">
                <div className="text-lg font-bold text-[#003580]">24/7</div>
                <div className="text-xs text-gray-400">{t('Qo\'llab-quvvatlash', 'Поддержка')}</div>
              </div>
            </div>
            ...
            <div className="mb-4">
              <div className="flex justify-between mb-1 text-xs text-gray-500">
                <span>{t('Integratsiya darajasi', 'Уровень интеграции')}</span>
                <span>{(idx + 1) * 20}%</span>
              </div>
            ...
            <div className="flex gap-3 mt-4">
              ...
              <Link 
                ...
                onClick={() => {
                  ...
                  toast.success(`${org.name} tizimida ro'yxatdan o'tish`);
                }}
              >
            ...
            <div className="flex gap-2 pt-3 mt-3 border-t border-blue-100">
              <button 
                onClick={() => {
                  toast.success(`${org.name} haqida ma'lumot yuborildi`);
                }}
                className="flex-1 text-xs text-gray-400 hover:text-[#003580] transition-colors py-1"
              >
                <i className="mr-1 fas fa-info-circle"></i> {t('Batafsil', 'Подробнее')}
              </button>
              <button 
                onClick={() => {
                ...
                className="flex-1 text-xs text-gray-400 hover:text-[#003580] transition-colors py-1"
              >
                <i className="mr-1 fas fa-share-alt"></i> {t('Ulashish', 'Поделиться')}
              </button>
            </div>
          </div>
  ...
  <div className="mt-12 text-center">
    <button 
      onClick={() => {
        toast.success(t('Barcha tizimlar yuklanmoqda', 'Все системы загружаются'));
      }}
    ...
    >
      <span>{t('Barcha tizimlarni ko\'rish', 'Посмотреть все системы')}</span>
...
`}</style>
        ...
        <div className="relative mt-12 overflow-hidden transition-all duration-500 shadow-2xl rounded-2xl hover:shadow-3xl group">
          ...
          <div className="absolute z-20 flex items-center gap-2 px-4 py-2 text-sm text-gray-700 transition-all duration-500 transform -translate-x-1/2 rounded-full shadow-lg opacity-0 bottom-4 left-1/2 bg-white/90 backdrop-blur-sm group-hover:opacity-100">
            ...
            <span>Jondor tumani, Buxoro viloyati</span>
          </div>
        </div>
      </div>
